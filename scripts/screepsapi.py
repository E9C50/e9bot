...
class API(object):
    ...
    def get(self, _path, **args):
        return self.req(requests.get, _path, params=args)

    def post(self, _path, **args):
        return self.req(requests.post, _path, json=args)

    def __init__(
        self, u=None, p=None, token=None, host=None, prefix=None, secure=True, ptr=False
    ):
    ...
    def steam_ticket(self, ticket, useNativeAuth=False):
        return self.post(
            "auth/steam-ticket", ticket=ticket, useNativeAuth=useNativeAuth
        )
    ...
    def room_terrain(self, room, encoded=False, shard=DEFAULT_SHARD):
        if encoded:
            return self.get(
                ...
                room=room,
                shard=shard,
                encoded=("1" if encoded else None),
            )
        else:
    ...
    def board_list(self, limit=10, offset=0, season=None, mode="world"):
        ...
        ret = self.get(
            "leaderboard/list", limit=limit, offset=offset, mode=mode, season=season
        )
        for d in ret["list"]:
    ...
    def check_unique_name(self, type, name=None, shard=DEFAULT_SHARD):
        return self.post(
            "game/check-unique-object-name", type=type, name=name, shard=shard
        )
    ...
    def place_spawn(self, room, name, x, y, shard=DEFAULT_SHARD):
        return self.post(
            "game/place-spawn", room=room, name=name, x=x, y=y, shard=shard
        )
    ...
    def add_object_intent(self, room, name, intent, shard=DEFAULT_SHARD):
        return self.post(
            "game/add-object-intent", room=room, name=name, intent=intent, shard=shard
        )

    def set_notify_attacked(self, _id, enabled=True, shard=DEFAULT_SHARD):
        return self.post(
            "game/set-notify-when-attacked", _id=_id, enabled=enabled, shard=shard
        )
    ...
    def connect(self):
        ...
        if "http_proxy" in self.settings and self.settings["http_proxy"] is not None:
            http_proxy_port = (
                ...
                if "http_proxy_port" in self.settings
                else 8080
            )
            self.ws.run_forever(
                http_proxy_host=self.settings["http_proxy"],
