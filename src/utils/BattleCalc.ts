export default class BattleCalc {
    ...
    public static calcPartTypeDamage(
        ...
        let power = BattleCalc.BODY_POWER[partType]
            ...
            return (
                ...
                _.sum(creep.body, (part) =>
                    part.type == partType && (part.hits || !checkHits)
                        ? part.boost
                            ? BattleCalc.BOOST_POWER[part.boost]
                            : 1
                        : 0,
                )
            )
        ...
        return (
            power *
            _.sum(creep.body, (part) =>
                part.type == partType ? (part.boost ? BattleCalc.BOOST_POWER[part.boost] : 1) : 0,
            )
        )
    }
    ...
    public static calcRangeDamage(creep: Creep, distance = 1, massAttack = false, checkHits = false) {
        return BattleCalc.calcPartTypeDamage(creep, RANGED_ATTACK, distance, massAttack, checkHits)
    }
    ...
    public static calcAttackDamage(creep: Creep, distance = 1, checkHits = false) {
        return BattleCalc.calcPartTypeDamage(creep, ATTACK, distance, false, checkHits)
    }
    ...
    public static calcWorkDamage(creep: Creep, distance = 1, checkHits = false) {
        return BattleCalc.calcPartTypeDamage(creep, WORK, distance, false, checkHits)
    }
        ...
        if (typeof checkHits === 'number') {
            ...
            return (
                ...
                _.sum([...creep.body].reverse().slice(0, checkHits), (part) =>
                    part.type == HEAL ? (part.boost ? BattleCalc.BOOST_POWER[part.boost] : 1) : 0,
                )
            )
        }

        return BattleCalc.calcPartTypeDamage(creep, HEAL, distance, rangeHeal, checkHits)
    }
    ...
    public static calcCreepDamage(creep: Creep, usedWork = true, distance = 1, massAttack = false, checkHits = false) {
        // 使用 work
        const work = usedWork ? BattleCalc.calcWorkDamage(creep, distance, checkHits) : 0
        // 如果用了 work 就没办法用 attack了
        const atk = work ? 0 : BattleCalc.calcAttackDamage(creep, distance, checkHits)
        return BattleCalc.calcRangeDamage(creep, distance, massAttack, checkHits) + work + atk
    }
    ...
    public static calcHealHoldRealDamage(creep: Creep, healPower: number, checkHits: boolean | number = false): number {
        ...
        const partDamageAndHits = creep.body.map((part, index) => {
            ...
            return [part.type === TOUGH ? hits / (BattleCalc.BOOST_POWER[part.boost!] || 1) : hits, hits]
        })
    ...
    public static calcDamageNeedHeal(creep: Creep, damage: number, checkHits: boolean | number = false) {
        ...
        for (let i = 0; i < creep.body.length; i++) {
            ...
            const boost = part.type === TOUGH && part.boost ? BattleCalc.BOOST_POWER[part.boost] : 1
            // 可以承受的伤害
